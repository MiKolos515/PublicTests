...
void KeysCheck(){
    ...
    UP = GetKeyState(VK_UP) & 0x8000;
    DOWN = GetKeyState(VK_DOWN) & 0x8000;
    LEFT = GetKeyState(VK_LEFT) & 0x8000;
    RIGHT = GetKeyState(VK_RIGHT) & 0x8000;
    if(W||UP){
...
int main()
{
    ...
    Symbol("Move with WSAD or arrows, exit with ESC. (F11-Fullscreen)\n\n",0x0F);

    Symbol("Provided to you by:\n \"",0x0F);
    Symbol("Szalony Kapelusznik",0x05);
    Symbol("\".\n\nDo you wish for \ndetailed textures?\n",0x0F);
    cin>>answer;

    if (answer=="1"||answer=="yes"||answer=="Yes"||answer=="y"||answer=="Y"||answer=="true"||answer=="TRUE") {
        SuperTextures = true;
    }
